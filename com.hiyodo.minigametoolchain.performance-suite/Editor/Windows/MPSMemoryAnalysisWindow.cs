...
using MiniGame.PerformanceSuite.Editor.Memory;
...
namespace MiniGame.PerformanceSuite.Editor.Windows
{
    ...
    public class MPSMemoryAnalysisWindow : EditorWindow
    {
        ...
        private static void ShowWindow()
        {
            ...
            MiniGameBranding.SetTitleIcon(window, "Memory Analysis");
            window.Show();
        }
        ...
        private readonly List<UnityEngine.Object> _rankingObjects = new List<UnityEngine.Object>();
        private readonly List<MPSReferenceChainLink> _referenceChains = new List<MPSReferenceChainLink>();
        private string _referenceChainTargetName;
        ...
        private void OnGUI()
        {
            ...
            MiniGameBranding.DrawHeader("内存分析", "实时趋势、iOS 内存预警与资源排名");
            DrawToolbar();
        ...
        private void DrawResourceRankings()
        {
            ...
            var grouped = _resourceRankings
                ...
                .OrderByDescending(g => g.Sum(x => x.entry.SizeBytes));

            bool chainEnabled = MiniGameLicenseManager.IsFeatureEnabled(MiniGameLicenseFeature.PerformanceSuiteMemoryDeepAnalysis);

            foreach (var group in grouped)
            {
                var totalBytes = group.Sum(x => x.entry.SizeBytes);
                EditorGUILayout.LabelField($"{group.Key}: {MPSMemoryAnalysisReport.FormatBytes(totalBytes)}", EditorStyles.boldLabel);

                foreach (var item in group.Take(TopAssetCount))
                {
                    EditorGUILayout.BeginHorizontal();
                    EditorGUILayout.LabelField(item.entry.Name, GUILayout.Width(200f));
                    EditorGUILayout.LabelField(MPSMemoryAnalysisReport.FormatBytes(item.entry.SizeBytes), GUILayout.Width(70f));

                    if (!string.IsNullOrEmpty(item.entry.Source))
                    {
                        EditorGUILayout.LabelField(item.entry.Source, EditorStyles.miniLabel, GUILayout.Width(130f));
                    }

                    if (chainEnabled && item.index < _rankingObjects.Count)
                    {
                        if (GUILayout.Button("链", GUILayout.Width(30f)))
                        {
                            AnalyzeReferenceChain(item.index);
                        }
                    }

                    EditorGUILayout.EndHorizontal();

                    if (!string.IsNullOrEmpty(item.entry.Detail))
                    {
                        EditorGUILayout.LabelField($"    {item.entry.Detail}", EditorStyles.miniLabel);
                    }
                }
        ...
        private void AnalyzeReferenceChain(int rankingIndex)
        {
            var target = rankingIndex < _rankingObjects.Count ? _rankingObjects[rankingIndex] : null;
            if (target == null) return;

            _referenceChainTargetName = $"{target.GetType().Name} '{target.name}'";
            _referenceChains.Clear();

            // 全量扫描可能耗时数百毫秒到数秒，加等待提示
            EditorUtility.DisplayProgressBar("引用链分析", $"正在扫描谁引用了 {_referenceChainTargetName}...", 0.5f);
            try
            {
                _referenceChains.AddRange(MPSReferenceChainAnalyzer.FindReferenceChains(target));
            }
            finally
            {
        ...
        private void DrawReferenceChainSection()
        {
            if (_referenceChains.Count == 0) return;

            EditorGUILayout.Space();
            EditorGUILayout.LabelField($"引用链：{_referenceChainTargetName}（{_referenceChains.Count} 条）", EditorStyles.boldLabel);

            int shown = 0;
            foreach (var chain in _referenceChains.OrderBy(c => c.Depth))
            {
                if (shown >= 30) break;
                EditorGUILayout.LabelField($"[深度 {chain.Depth}] {chain.DisplayPath}", EditorStyles.miniLabel);
                shown++;
            }

            if (_referenceChains.Count > 30)
            {
                EditorGUILayout.LabelField($"……其余 {_referenceChains.Count - 30} 条省略", EditorStyles.miniLabel);
            }
        }
        ...
        private void RefreshResourceRankings()
        {
            ...
            // 按大小排序，同时保持 _rankingObjects 与 _resourceRankings 索引对齐
            var order = Enumerable.Range(0, _resourceRankings.Count)
                .OrderByDescending(i => _resourceRankings[i].SizeBytes)
                .ToList();
            var sortedEntries = order.Select(i => _resourceRankings[i]).ToList();
            var sortedObjects = order.Select(i => _rankingObjects[i]).ToList();
            _resourceRankings.Clear();
            _resourceRankings.AddRange(sortedEntries);
            _rankingObjects.Clear();
            _rankingObjects.AddRange(sortedObjects);
        }
        ...
        private void CollectAssetsOfType<T>(string typeName) where T : UnityEngine.Object
        {
            ...
            foreach (var obj in objects)
            {
                ...
                _resourceRankings.Add(new MPSMemoryAnalysisAssetEntry
                {
                    ...
                    Name = obj.name,
                    SizeBytes = size,
                    Source = MPSAssetSourceTracer.GetSource(obj),
                    Detail = MPSAssetSourceTracer.GetDetail(obj)
                });
                _rankingObjects.Add(obj);
            }
        }
