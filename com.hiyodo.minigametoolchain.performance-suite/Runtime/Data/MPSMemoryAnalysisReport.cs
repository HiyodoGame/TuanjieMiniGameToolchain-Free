...
namespace MiniGame.PerformanceSuite.Runtime.Data
{
    ...
    public class MPSMemoryAnalysisAssetEntry
    {
        ...
        /// <summary>
        /// 资源来源（AssetBundle: xxx / Resources / 场景 / 项目资源 / 动态生成）。
        /// 由编辑器侧填充，运行时快照中可能为空。
        /// </summary>
        public string Source;

        /// <summary>
        /// 资源明细（纹理尺寸格式 / Mesh 顶点数据 / 音频声道等）。
        /// </summary>
    ...
    public class MPSMemoryAnalysisReport
    {
        ...
        public void SaveMarkdown(string path)
        {
            ...
            sb.AppendLine();
            sb.AppendLine("| 类型 | 名称 | 大小 | 来源 | 明细 |");
            sb.AppendLine("|------|------|------|------|------|");
            foreach (var entry in TopAssets.Take(50))
            {
                sb.AppendLine($"| {entry.Type} | {entry.Name} | {FormatBytes(entry.SizeBytes)} | {entry.Source ?? "-"} | {entry.Detail ?? "-"} |");
            }
